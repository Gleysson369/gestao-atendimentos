...
public interface AtendimentoRepository extends JpaRepository<Atendimento, Long>, JpaSpecificationExecutor<Atendimento> {
    ...
    @Query("SELECT COUNT(a) FROM Atendimento a WHERE a.usuarioAtendente = :usuarioAtendente AND EXTRACT(MONTH FROM a.dataHoraAtendimento) = EXTRACT(MONTH FROM CURRENT_TIMESTAMP) AND EXTRACT(YEAR FROM a.dataHoraAtendimento) = EXTRACT(YEAR FROM CURRENT_TIMESTAMP)")
    Long countMonthlyByUsuarioAtendente(@Param("usuarioAtendente") Usuario usuarioAtendente);
    ...
    //Novo método quantidade de CRS.
    @Query("SELECT COUNT(a) FROM Atendimento a WHERE a.numeroCRS IS NOT NULL AND a.numeroCRS != '' AND EXTRACT(MONTH FROM a.dataHoraAtendimento) = EXTRACT(MONTH FROM CURRENT_TIMESTAMP) AND EXTRACT(YEAR FROM a.dataHoraAtendimento) = EXTRACT(YEAR FROM CURRENT_TIMESTAMP)")
    Long countMonthlyCrs();
    
    @Query("SELECT COUNT(a) FROM Atendimento a WHERE a.numeroCRS IS NOT NULL AND a.numeroCRS != '' AND EXTRACT(DAY FROM a.dataHoraAtendimento) = EXTRACT(DAY FROM CURRENT_TIMESTAMP) AND EXTRACT(MONTH FROM a.dataHoraAtendimento) = EXTRACT(MONTH FROM CURRENT_TIMESTAMP) AND EXTRACT(YEAR FROM a.dataHoraAtendimento) = EXTRACT(YEAR FROM CURRENT_TIMESTAMP)")
    Long countDailyCrs();

    //Novo método média móvel.

    @Query("SELECT EXTRACT(MONTH FROM a.dataHoraAtendimento), EXTRACT(YEAR FROM a.dataHoraAtendimento), COUNT(a.id) FROM Atendimento a WHERE a.dataHoraAtendimento >= :seisMesesAtras GROUP BY EXTRACT(MONTH FROM a.dataHoraAtendimento), EXTRACT(YEAR FROM a.dataHoraAtendimento) ORDER BY EXTRACT(YEAR FROM a.dataHoraAtendimento) ASC, EXTRACT(MONTH FROM a.dataHoraAtendimento) ASC")
    List<Object[]> countMonthlyAtendimentosLastSixMonths(@Param("seisMesesAtras") LocalDateTime seisMesesAtras);
